...
SuperColliderTrackEditor::render_to_timeline ()
{
	...
	for (std::vector<std::shared_ptr<Region> >::const_iterator i = clips.begin (); i != clips.end (); ++i) {
		...
		double const duration = static_cast<double> (clip->length_samples ()) / session.sample_rate ();
		if (duration <= 0.0) {
			continue;
		}

		std::string const base_name = string_compose ("scardour-%1-%2", sct->id ().to_s (), clip->id ().to_s ());
		std::string const render_path = Glib::build_filename (session.session_directory ().sound_path (), base_name + ".wav");
		std::string const script_path = Glib::build_filename (session.session_directory ().sound_path (), base_name + ".scd");

		std::ofstream script (script_path.c_str (), std::ios::out | std::ios::trunc);
		if (!script) {
			_status_label.set_text (_("Render status: could not write SuperCollider render script"));
			return;
		}

		script
			<< "(\n"
			<< "var outputPath = " << sc_string_literal (render_path) << ";\n"
			<< "var sampleRate = " << session.sample_rate () << ";\n"
			...
			<< "    ~ardourTrackId = " << sc_string_literal (sct->id ().to_s ()) << ";\n"
			<< "    ~ardourTrackName = " << sc_string_literal (sct->name ()) << ";\n"
			<< "    ~ardourSynthDef = " << sc_string_literal (sct->supercollider_synthdef ()) << ";\n"
			<< "    ~ardourRegionId = " << sc_string_literal (clip->id ().to_s ()) << ";\n"
			<< "    ~ardourRegionName = " << sc_string_literal (clip->name ()) << ";\n"
			<< "    ~ardourRegionStart = 0;\n"
			<< "    ~ardourRegionEnd = " << clip->length_samples () << ";\n"
			<< "    ~ardourRegionDuration = duration;\n"
			<< "    ~ardourTrackGroup = Group.tail(s);\n"
			<< sct->supercollider_source () << "\n"
		...
		if (!ok || exit_status != 0 || !g_file_test (render_path.c_str (), G_FILE_TEST_EXISTS)) {
			std::string reason = _("SuperCollider render failed");
			...
			_status_label.set_text (string_compose (_("Render status: %1"), reason));
			return;
		}
		...
		SourceList sources;
		sources.push_back (SourceFactory::createExternal (DataType::AUDIO, session, render_path, 0, Source::Flag (AudioFileSource::NoPeakFile), false));
		sources.push_back (SourceFactory::createExternal (DataType::AUDIO, session, render_path, 1, Source::Flag (AudioFileSource::NoPeakFile), false));

		if (!sources[0] || !sources[1]) {
			_status_label.set_text (_("Render status: rendered file could not be imported"));
			return;
		}
		...
		plist.add (ARDOUR::Properties::name, string_compose (_("SC Render: %1"), clip->name ()));
		...
		render_playlist->add_region (rendered_region, clip->position ());
